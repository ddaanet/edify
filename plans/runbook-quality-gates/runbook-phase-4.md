...
**Behavior:**
- Process cycles in document order, maintaining `created_names` set (function/module names created in GREEN `**Changes:**` sections)
- For each cycle: first check its GREEN `**Changes:**` section for `Action: Create` entries → extract module/function names created; add to `created_names`
- Then check its RED `**Expected failure:**` line for function/module names referenced
...
**Approach:** Parse `**Expected failure:**` with regex to extract module/function name (e.g., `ImportError` on `src.module` → extract `src.module` or `module`). Parse GREEN `**Changes:**` `Action: Create` entries for file stem names. Process GREEN before RED within same cycle (RED is the pre-implementation test; prior cycle GREENs establish state).

**Changes:**
- File: `agent-core/bin/validate-runbook.py`
  Action: Add `check_red_plausibility(content, path)` with GREEN-before-RED processing; wire to handler
...
**Why it fails:** 4.1 happy-path implementation sets up the data structures but may not implement the violation detection branch.
...
**Approach:** After collecting `created_names` from all prior cycles' GREENs (and current cycle's GREEN), check RED `**Expected failure:**`. If failure type is `ImportError`/`ModuleNotFoundError` AND the name appears in `created_names` → violation. `sys.exit(1)` with violations.
...
- Report `Ambiguous` section names the function and explains it exists but RED tests different behavior
