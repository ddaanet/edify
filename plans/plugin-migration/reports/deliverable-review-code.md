...
- [Major] :30,35 — conformance (D-1): Package installed is `edify==$EDIFY_VERSION` but D-1 states the Python package name is `edify`. The pyproject.toml still shows `name = "edify"` so this is currently correct at runtime, but it diverges from the naming decision. If the rename has been deferred, this should be documented; if it hasn't, the package name should be `edify`.
...
**Major finding:** Package name `edify` in the install command diverges from D-1's stated naming (`edify`). This needs clarification — either the rename is deferred (document it) or the package name should be updated.
