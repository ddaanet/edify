...
1. **Keyword overlap %** (`src/claudeutils/recall/relevance.py:22`) — 0.3 default. Fraction of entry keywords matching session keywords. Controls which entries surface as "relevant."

2. **Fuzzy match threshold** (`src/claudeutils/validation/memory_index.py:127`, `memory_index_checks.py:222`) — 50.0. Score threshold for matching index entry keys to semantic headers. Controls orphan detection and collision checking.
...
4. **Content words** (`src/claudeutils/validation/learnings.py:147`) — Minimum 2 content words after prefix in decision headings. Controls specificity floor for trigger titles.
...
- `src/claudeutils/recall/index_parser.py` — keyword extraction, IndexEntry model
- `src/claudeutils/validation/memory_index.py` — index validation with fuzzy matching
- `src/claudeutils/validation/memory_index_checks.py` — collision and duplicate checks
- `src/claudeutils/validation/learnings.py` — content word validation
