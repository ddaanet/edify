...
- Acceptance: `plans/foo/` contains only `brief.md` → `edify _worktree ls` shows `[briefed] → /design plans/foo/brief.md`
...
Fix is in `src/edify/validation/planstate.py`. Brief-only is a new state in the inference chain, inserted before the requirements.md fallback.
