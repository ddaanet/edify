...
1. **`cached_count_tokens_for_file` silently skips cache for empty files**
   - Location: `src/edify/token_cache.py:105-107`
...
2. **Duplicated cache-init pattern between `tokens.py` and `tokens_cli.py`**
   - Location: `src/edify/tokens.py:186-192`, `src/edify/tokens_cli.py:62-71`
...
1. **Trivial docstrings on SQLAlchemy model and `Base`**
   - Location: `src/edify/token_cache.py:17`, `21`
...
2. **`create_cache_engine` docstring with Args/Returns block for a simple function**
   - Location: `src/edify/token_cache.py:71-83`
...
3. **`cached_count_tokens_for_file` docstring longer than the function**
   - Location: `src/edify/token_cache.py:86-117`
...
4. **`get_default_cache` docstring restates return type**
   - Location: `src/edify/token_cache.py:120-130`
...
- `src/edify/token_cache.py:88-93` — Removed empty-file early return from `cached_count_tokens_for_file`; added `try/except (PermissionError, OSError, UnicodeDecodeError) → FileReadError` wrapping around `path.read_text()` to match `count_tokens_for_file`'s error contract. Empty files now flow through to `count_tokens_for_file` which returns 0.
- `src/edify/tokens_cli.py` — Removed duplicated cache-init block; `handle_tokens` now delegates to `count_tokens_for_files`. Removed now-unused `logging`/`logger`, `TokenCount`, and `count_tokens_for_file` imports.
- `src/edify/tokens.py:186` — Added `noqa: PLC0415, I001` to the deferred import (circular import avoidance — `token_cache.py` imports from `tokens.py`).
- `src/edify/token_cache.py` — Replaced trivial docstrings with minimal-informative ones (`Base`, `TokenCacheEntry`); trimmed `create_cache_engine` and `cached_count_tokens_for_file` Args/Returns blocks to single-line summaries; removed `get_default_cache` Returns block.
- `tests/conftest.py` — Updated `mock_token_counting` and `cli_base_mocks` fixtures: patch `edify.token_cache.get_default_cache` (not `tokens.py`) with in-memory cache, patch `edify.token_cache.count_tokens_for_file` (not `tokens_cli`). Moved imports to top-level.
- `tests/test_cli_tokens.py` — Updated three tests that directly patched `edify.tokens_cli.count_tokens_for_file` to use `edify.token_cache.count_tokens_for_file`. Added `TokenCache`/`create_cache_engine` to top-level imports.
...
| `platformdirs.user_cache_dir("edify") / "token_cache.db"` | Satisfied | `get_default_cache()` uses exact path |
...
- Deferred import (`from edify.token_cache import ...` inside function body) avoids circular import — token_cache imports from tokens.py.
