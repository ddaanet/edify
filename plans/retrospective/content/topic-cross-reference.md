...
| 2025-11-23 | scratch/box-api | `0bbdbf8` | AGENTS.md with session-specific rules. "Include any feedback that is general enough in scope" — filtering criterion for what to remember. |
| 2026-01-12 | scratch/home | `e31a4c0` | File organization table with session.md, design-decisions.md. Memory distributed across file types by content. |
| 2026-01-12 | scratch/pytest-md | `69bf88b` | Session log design — structured conversation state persistence. |
...
**Arc:** Human-written rules (rules) → agent retrospective self-update (oklch-theme) → structured file taxonomy (scratch/home) → shared infrastructure (agent-core) → memory-index + recall system (claudeutils).
...
| 2025-10-12 | scratch/emojipack | `9cc5c62` | "#auto Proceed autonomously without asking until all tasks complete" — same pattern. No evaluation before action. |
| 2025-10-25 | tuick | `7d2fa67` | Same emojipack template, no pushback mechanisms. |
| 2025-12-01 | oklch-theme | `64cbf8f` | "LLM Limitation Awareness" — first metacognitive rules. "Flag uncertainty and request validation." This IS proto-pushback: agent asked to identify its own weaknesses. |
...
**Arc:** "Proceed autonomously" (no pushback) → "Flag uncertainty" (oklch-theme, proto-pushback) → "Cognitive protocols" (tuick, overengineered) → removed (tuick, too heavy) → structural enforcement (scratch/home) → formalized pushback protocol (claudeutils).

## Topic 3: Deliverable-Review Origins

| Date | Repo | Hash | Evidence |
|------|------|------|----------|
| 2025-10-18 | scratch/emojipack | `996158f` | "Red-Green does not apply when removing code" — first quality gate exception. Review of process, not output. |
...
| 2026-03-10 | devddaanet | `4ffb1ea` | "Re-review: all findings resolved, delivered" — full review loop completed. |

**Arc:** Ad-hoc quality rules → structured process review → deliverable-review skill in production (devddaanet is the strongest external validation).
...
| 2025-11-23 | scratch/box-api | `791a962` | "Flesh out AGENTS.md (incorporate relevant rules for tuick)" — cross-project rule import. Pattern: take rules from one project, adapt for another. |
...
| Date | Repo | Hash | Evidence |
|------|------|------|----------|
| 2025-10-12 | scratch/emojipack | `9cc5c62` | "`just agent` before every commit" — first commit gate. Mechanical enforcement via recipe. |
| 2025-10-25 | tuick | `7d2fa67` | Same commit gate pattern propagated. |
| 2025-11-23 | scratch/box-api | `0bbdbf8` | Agent-specific `just` recipes: `just agent`, `just agent-check`, `just agent-test`. Separation of agent and human commands — structural role differentiation. |
| 2025-12-05 | scratch/box-api | `b3aa473` | "One-letter commands" — shorthand for frequent agent operations. |
| 2026-01-03 | scratch/pytest-md | `fd939eb` | `.claude/settings.json` — platform-level configuration as enforcement. |
| 2026-01-13 | scratch/home | `deb0d91` | Orchestrator constraints: "Allowed/Delegate" model. Structural separation of what orchestrator can and cannot do. Proto-gating. |
...
**Arc:** `just agent` gate (emojipack) → agent-specific recipes (box-api) → platform config (pytest-md) → orchestrator constraints (scratch/home) → shared infrastructure (agent-core) → D+B tool-call anchoring → PreToolUse hooks (claudeutils).
