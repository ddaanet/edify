...
**Problem:** System invariants documented as learnings need to be referenceable from recall artifacts. Current `edify _recall resolve` only processes decision file entries (`agents/decisions/`). Learning keys (e.g., `when invariant: worktree cap`) are unresolvable.
