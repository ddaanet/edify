...
class TestNewDirectives:
    ...
    def test_p_directive_dual_output(self) -> None:
        ...
        assert result["systemMessage"] == "[PENDING] Capture task, do not execute."
    ...
    def test_b_brainstorm_directive(self) -> None:
        ...
        assert "[BRAINSTORM]" in result["systemMessage"]
    ...
    def test_q_quick_directive(self) -> None:
        ...
        assert "[QUICK]" in result["systemMessage"]
        additional_context = result["hookSpecificOutput"]["additionalContext"]
        assert (
    ...
    def test_learn_directive(self) -> None:
        ...
        assert "[LEARN]" in result["systemMessage"]
        additional_context = result["hookSpecificOutput"]["additionalContext"]
    ...
    def test_directive_long_form_aliases(self) -> None:
        ...
        assert "[BRAINSTORM]" in call_hook("brainstorm: ideas")["systemMessage"]
        assert "[QUICK]" in call_hook("question: how does X work")["systemMessage"]
        assert "[PENDING]" in call_hook("pending: new task name")["systemMessage"]
