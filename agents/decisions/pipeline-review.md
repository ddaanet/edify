...
**Correct pattern:** "If absent, do lightweight recall" — read `memory/MEMORY.md`, identify review-relevant entries (quality patterns, failure modes), Read them. The Tier 1/2 sections in the runbook skill already have this fallback.
...
**Correct pattern:** Read the artifact to identify WHICH decisions matter, then Read those files to load WHAT they say (full decision section content). Both steps required in new sessions.

**Evidence:** RCA identified the root cause.
