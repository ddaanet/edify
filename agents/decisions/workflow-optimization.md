...
**Decision:** All tiers (1/2/3) must end with `/handoff:handoff` → `/commit-commands:commit` chain, never bare `/commit-commands:commit`.

**Anti-pattern:** Tier 1/2 skip handoff because "no session restart needed"

**Correct pattern:** Always chain `/handoff:handoff` → `/commit-commands:commit` — handoff captures session context and learnings regardless of tier
...
**Decision:** When `/handoff:handoff` is followed by `/commit-commands:commit` in a continuation chain, session.md reflects post-commit state.
...
**Example:** Oneshot assessed simple/moderate/complex, then the planning skill re-assessed Tier 1/2/3 — same function, different labels.
...
**Evidence:** 8-round design discussion explored @ref preload → SessionStart hook → scripted gate → all dropped. Final: a priming step that chain-calls the recall pass.
