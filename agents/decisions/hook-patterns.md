...
**Workaround:** Use `@`.claude/handoff-task.md` in CLAUDE.md for task context (already loaded).
...
**Anti-pattern:** Hook command in settings.json uses a relative path (e.g. `.claude/hooks/<hook>.py`). After agent cwd drifts (e.g., `cd subdir && ...`), the relative path resolves against the new cwd — hook script not found. Hook errors are non-blocking in Claude Code, so the guarded command proceeds without the hook.
