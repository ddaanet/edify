...
- 5 skills: `/design`, `/runbook`, `/orchestrate`, `/review`, `/claude-md-management:revise-claude-md`
...
- `/runbook` skill with 5-phase execution (includes automated review)
...
- Enforced test-first methodology via /runbook skill
...
- **Sequence:** Implement directly → vet agent → apply fixes → `/handoff:handoff` → `/commit-commands:commit`
...
**Haiku-specific:** Handoff uses `/handoff-haiku` not `/handoff:handoff` (no learnings judgment).
