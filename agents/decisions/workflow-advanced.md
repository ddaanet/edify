...
**Decision:** Learnings → learnings.md → consolidation → permanent doc → index entry.
...
**Correct pattern:** Keep the artifact's relevance notes aligned with the section headings they point at. Verify by Reading the referenced file before committing.
...
**Anti-pattern:** Naming memory entries after the anti-pattern, outcome, or self-assessment ("When synthesizing ungrounded methodology", "When deliverable review catches drift", "When resuming killed agents").
...
**Same principle as naming a memory entry:** discovery and recall over precision.
...
**Correct pattern:** Embed critical knowledge in loaded context (CLAUDE.md @-references for always-available rules; `memory/MEMORY.md` and the files it indexes for on-demand recall).
...
**Implication:** Pipeline recall is effective even on the moderate path (no formal requirements): derive domain keywords from user request, match against `memory/MEMORY.md`, follow cross-references to discover relevant decisions the user didn't explicitly mention.
...
**Exemption:** Volatile session state (`.claude/handoff-task.md`) is exempt — `#PNDNG` token expansion runs in precommit.
...
**Correct pattern:** File paths only. The artifact lists memory and decision file paths with 1-line relevance notes. Downstream consumers Read those files to get current content. No staleness, no excerpt duplication.
...
- **Submit:** `/handoff:handoff` then `/commit-commands:commit` (or `hc`)
