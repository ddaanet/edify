...
**Correct pattern:** Anchor with a tool call that proves work happened. `edify _recall resolve` is the canonical gate anchor: it's a Bash call (unskippable), requires trigger knowledge (forces prior Read of memory-index), and produces output (proves resolution). One tool-call anchor is sufficient — passphrase/proof-of-Read mechanisms are redundant when the resolution tool proves both.
...
**Correct pattern:** Verify the tool's mechanism matches the gate's runtime state. The right anchor is the tool called on the positive path (`edify _recall resolve`). Null mode (`edify _recall resolve null`) provides the negative path at equal cost — silent exit 0, no output, proves gate was reached.
