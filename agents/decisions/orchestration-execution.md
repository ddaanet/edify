...
**Decision Date:** 2026-02-23, superseded 2026-08-13

**Decision:** Dispatch a standing agent (`artisan`, `test-driver`, `corrector`) with the path to a step file. The step file names the design, outline, and recall artifacts the executor needs; the orchestrator's prompt carries paths, never content.

**Superseded:** bespoke per-plan agent definitions (`<plan>-task`, `<plan>-corrector`, `<plan>-tester`, `<plan>-implementer`) generated by prepare-runbook.py into `.claude/agents/`. That scheme fought the platform — generated agents are not discoverable as `subagent_type` values until session restart (see `project-config.md`, "When Custom Agents Need Session Restart For Discoverability"), which forced a restart boundary into the middle of the pipeline and made agent-type substitution a live failure mode.
...
**Context priming:** Sub-agents don't share parent context. Each NEW agent must self-prime by Reading the files named in the recall artifact. Resumed agents already have this context.
