...
**Pattern:** A specific requirement (e.g., "post-execution comparison point") that can't map to any existing pipeline stage reveals a structural gap. The pipeline state machine (/requirements → /design → /runbook → /orchestrate → /deliverable-review → /commit-commands:commit) has no stage for execution-ready work between /design and /handoff:handoff.
