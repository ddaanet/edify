...
**Trigger:** When adding new kind of state to versioned content, design extension point in /commit-commands:commit skill to check for that state.
...
**Correct pattern:** Only commit when user explicitly requests (`/commit-commands:commit`, `ci`, `xc`, `hc`).
...
**Process fix:** Skill fixes outlined for `/design` and `/runbook`.

**Impact:** Implementation matches design intent without interpretation drift.

### .When Writing Memory Index Entry Keys

**Anti-pattern:** Adding header titles to `memory/MEMORY.md` and claiming "entries exist".
...
**Example:** the memory index's word-count check changed from warning to hard error, exposed 62 violations immediately.
...
**Script location:** `plugin/bin/batch-edit.py` (not restored in the 2026-08 revival)
...
**Integration:** Parse `just precommit` output → structured error entries → extract error codes/categories → map to recall domain keywords → inject the matching entries from `memory/MEMORY.md`.
