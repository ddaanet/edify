...
- Consolidate `memory/MEMORY.md` — 28.8KB against Claude Code's 24.4KB loader cutoff, so tail entries never reach a session; the gitlore hook wants it under 17.1KB. Classify each line WHEN/HOW/acted-inline, then retire and relocate entries rather than reword them
- The `ddaanet` tier's own `MEMORY.md` has crossed the same 24.4KB cutoff (confirmed during two tier merges this session) — needs the same retirement treatment, one level up
- Exercise the revived pipeline end-to-end (design → runbook → orchestrate), the first real exercise of the rewired step-file and manifest shapes
